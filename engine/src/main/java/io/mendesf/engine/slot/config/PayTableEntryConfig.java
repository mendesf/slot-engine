...
public record PayTableEntryConfig(
    SymbolConfig symbol,
    List<TierConfig> tiers
) {
    public PayTableEntryConfig(
        PayTableEntryConfigData data,
        Map<String, SymbolConfig> symbolsByName
    ) {
        this(
            resolveSymbol(data, symbolsByName),
            data.tiers().stream()
                .map(TierConfig::new)
                .toList()
        );
    ...
    private static SymbolConfig resolveSymbol(
        PayTableEntryConfigData data,
        Map<String, SymbolConfig> symbolsByName
    ) {
        ...
        if (symbol == null) {
            throw new SlotConfigReferenceException(
                "Symbol '" + data.symbol() + "' not found for paytable entry."
            );
        }
        return symbol;
    }
