...
public record EngineConfig(BetConfig bet, List<SymbolConfig> symbols, List<ReelConfig> reels,
		List<DisplayConfig> displays, List<PayTableConfig> payTables, List<StageConfig> stages) {
	...
	public static EngineConfig from(EngineConfigData data) {
		...
		// 2) Symbols
		List<SymbolConfig> symbols = data.symbols().stream().map(SymbolConfig::new).toList();

		Map<String, SymbolConfig> symbolByName = symbols.stream()
				.collect(Collectors.toMap(SymbolConfig::name, Function.identity()));

		// 3) Reels
		List<ReelConfig> reels = data.reels().stream().map(r -> new ReelConfig(r, symbolByName)).toList();

		Map<String, ReelConfig> reelByName = reels.stream()
				.collect(Collectors.toMap(ReelConfig::name, Function.identity()));

		// 4) Displays
		List<DisplayConfig> displays = data.displays().stream().map(d -> new DisplayConfig(d, reelByName)).toList();

		Map<String, DisplayConfig> displayByName = displays.stream()
				.collect(Collectors.toMap(DisplayConfig::name, Function.identity()));

		// 5) PayTables
		List<PayTableConfig> payTables = data.payTables().stream().map(pt -> new PayTableConfig(pt, symbolByName))
				.toList();
		...
		// 6) Stages
		List<StageConfig> stages = data.stages().stream().map(s -> new StageConfig(s, displayByName, payTableByName))
				.toList();

		return new EngineConfig(bet, List.copyOf(symbols), List.copyOf(reels), List.copyOf(displays),
				List.copyOf(payTables), List.copyOf(stages));
	}
